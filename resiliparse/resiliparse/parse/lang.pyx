...
cdef uint32_t[200] VEC_EN = [4, 2, 3, 4, 3, 2, 4, 3, 3, 2, 4, 2, 1, 1, 3, 3, 6, 3, 4, 2, 3, 3, 2, 2, 7, 2, 2, 6, 6, 3,
...
cdef uint32_t[200] VEC_ML = [4, 6, 4, 2, 2, 3, 2, 2, 2, 3, 2, 2, 3, 2, 3, 5, 2, 1, 2, 2, 2, 2, 2, 6, 2, 3, 1, 1, 2, 3,
...
cdef uint32_t[200] VEC_HI = [5, 3, 3, 2, 4, 4, 4, 2, 3, 5, 3, 4, 3, 2, 3, 4, 2, 2, 2, 3, 4, 1, 2, 7, 2, 2, 3, 4, 5, 5,
...
cdef uint32_t[200] VEC_TA = [4, 10, 16, 16, 5, 1, 1, 19, 9, 12, 13, 14, 10, 4, 12, 1, 4, 4, 2, 2, 1, 2, 3, 2, 6, 4, 3,
...
cdef uint32_t[200] VEC_PT = [10, 2, 2, 4, 2, 3, 2, 4, 4, 3, 2, 2, 2, 2, 2, 2, 4, 3, 3, 5, 6, 2, 3, 2, 4, 2, 2, 2, 4, 3,
...
cdef uint32_t[200] VEC_FR = [4, 2, 2, 4, 2, 3, 2, 3, 3, 3, 3, 2, 1, 2, 4, 3, 4, 3, 4, 6, 4, 2, 4, 2, 4, 2, 3, 5, 5, 3,
...
cdef uint32_t[200] VEC_NL = [4, 1, 3, 4, 3, 3, 2, 2, 2, 2, 3, 2, 1, 2, 4, 3, 3, 4, 2, 5, 4, 4, 4, 5, 6, 4, 4, 2, 5, 3,
...
cdef uint32_t[200] VEC_ES = [10, 1, 2, 4, 3, 4, 2, 2, 4, 2, 3, 3, 2, 1, 2, 2, 4, 4, 4, 4, 7, 2, 4, 2, 4, 3, 3, 5, 4, 3,
...
cdef uint32_t[200] VEC_EL = [1, 3, 3, 4, 1, 1, 3, 2, 5, 3, 3, 1, 1, 4, 3, 4, 3, 3, 3, 2, 3, 4, 2, 4, 4, 2, 5, 3, 3, 3,
...
cdef uint32_t[200] VEC_RU = [3, 5, 4, 8, 5, 5, 5, 3, 6, 3, 5, 3, 2, 2, 2, 3, 4, 4, 2, 3, 2, 2, 4, 1, 2, 5, 2, 2, 3, 2,
...
cdef uint32_t[200] VEC_DK = [4, 2, 3, 3, 2, 2, 1, 2, 2, 2, 3, 2, 3, 2, 4, 2, 4, 4, 2, 7, 6, 6, 4, 5, 5, 3, 5, 2, 6, 4,
...
cdef uint32_t[200] VEC_IT = [9, 3, 2, 5, 3, 3, 2, 4, 3, 2, 3, 2, 2, 2, 9, 3, 4, 3, 5, 5, 6, 2, 3, 2, 5, 2, 3, 4, 5, 3,
...
cdef uint32_t[200] VEC_TR = [4, 1, 2, 5, 2, 2, 2, 4, 3, 2, 4, 2, 3, 3, 6, 2, 4, 6, 2, 3, 4, 3, 3, 3, 7, 2, 3, 2, 3, 2,
...
cdef uint32_t[200] VEC_SV = [3, 2, 3, 5, 2, 2, 2, 3, 2, 2, 2, 4, 3, 2, 3, 2, 4, 5, 2, 6, 5, 4, 2, 4, 6, 3, 4, 2, 6, 3,
...
cdef uint32_t[200] VEC_AR = [7, 5, 7, 2, 6, 4, 5, 4, 3, 3, 10, 3, 3, 4, 2, 3, 2, 3, 7, 6, 10, 20, 13, 11, 6, 9, 6, 17,
...
cdef uint32_t[200] VEC_DE = [2, 3, 4, 3, 2, 5, 3, 4, 5, 2, 3, 3, 1, 1, 2, 5, 4, 5, 2, 5, 3, 4, 3, 3, 5, 3, 4, 2, 4, 2,
...
cdef uint32_t[200] VEC_KN = [3, 4, 5, 2, 1, 3, 2, 11, 3, 3, 3, 1, 1, 4, 3, 2, 3, 2, 2, 2, 4, 2, 6, 3, 3, 3, 5, 1, 2, 3,
...
cdef vector[uint32_t] str_to_vec(str train_text, size_t vec_len=N_LANGS):
    cdef long hash2
    cdef long hash3
    cdef long hash4
    cdef long hash5
    ...
    cdef vector[uint32_t] count_vec
    count_vec.resize(vec_len)
    ...
    for uchar in train_text:
        ...
        count_vec[hash(&uchar, 1) % count_vec.size()] += 1
        if i >= 1:
            count_vec[hash(ngram2, 2) % count_vec.size()] += 1
        if i >= 2:
            count_vec[hash(ngram3, 3) % count_vec.size()] += 1
        if i >= 3:
            count_vec[hash(ngram4, 4) % count_vec.size()] += 1
        if i >= 4:
            count_vec[hash(ngram5, 5) % count_vec.size()] += 1
    ...
    cdef size_t j
    if i > 0:
        for j in range(count_vec.size()):
            count_vec[j] = count_vec[j] * count_vec.size() // i

    return count_vec


cdef size_t cmp_oop_ranks(const uint32_t* vec1, const uint32_t* vec2, size_t size):
    cdef size_t rank = 0
    cdef size_t i
    for i in range(size):
        if vec1[i] > vec2[i]:
...
cpdef detect_fast(str text, size_t cutoff=1000):
    ...
    cdef vector[uint32_t] text_vec = str_to_vec(text, LANG_VEC_SIZE)
